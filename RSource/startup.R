...
finaldf <- read.csv("C:/Users/jaeyo/Downloads/youtube-statistics/CSV/editdf.csv")
...
finaldf <- finaldf %>% 
  mutate(
    LTVGroup = case_when(
      !is.na(LTV) & LTV > 0     & LTV <= 0.015 ~ "0~0.015 LTV",
      !is.na(LTV) & LTV > 0.015 & LTV <= 0.03  ~ "0.015~0.03 LTV",
      !is.na(LTV) & LTV > 0.03  & LTV <= 0.06  ~ "0.03~0.06 LTV",
      !is.na(LTV) & LTV > 0.06                 ~ "0.06~ LTV",
      !is.na(LTV) & LTV == 0                    ~ "Likes = 0"
    )
  )
...
finaldf <- finaldf %>% 
  mutate(
    LTVGroup = factor(
      LTVGroup,
      levels = c("Likes = 0", "0~0.015 LTV", "0.015~0.03 LTV", "0.03~0.06 LTV", "0.06~ LTV")
    )
  )
...
table1::table1(~LTVGroup, data = finaldf, na.rm = TRUE, digits = 2, format.number = TRUE)

...
table1::table1(~ViewsLog + Likes + Category  + DiffGroup + LTVGroup, data = finaldf, na.rm = TRUE, digits = 10, format.number = TRUE)

save_html(t1, file = "C:/Users/jaeyo/Downloads/youtube-statistics/Tables/VideoLengthGroup.html")

table1::table1(~Category, data = finaldf, na.rm = TRUE, digits = 2, format.number = TRUE)

table1::table1(~Likes, data = finaldf, na.rm = TRUE, digits = 10, format.number = TRUE)
